...
export default function AddDialog({ handleClose, show }) {
    ...
    const sendDone = () => {
        if (firstName !== "" && lastName !== "" && nic !== "" && contactNumber !== "" && address !== "" && userType !== "") {
            const data = { firstName, middleName,lastName,nic,contactNumber,address,userType };
            clearInputs();
        ...
        <Model show={show}>
            <DialogBox>
                ...
                <FlexRow>
                    ...
                    <Button onClick={sendCancel} color="danger">Cancel</Button>
                </FlexRow>
